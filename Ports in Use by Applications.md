...
| ehr-backend             | `C:\Users\yinka\Documents\ehr-backend`          | **5000** (or env PORT)  | Node/Express backend for MediForge.                                       | `node server.js` or nodemon. server.js sets `const PORT = process.env.PORT || 5000` |
...
| MediForge              | `C:\Users\yinka\Documents\MediForge`           | **5500**                | EHR UI / framework (often served with Python http.server or VS Code Live Server). | Typical: `python -m http.server 5500` or equivalent. See READMEs and HOW-TO-USE. |
