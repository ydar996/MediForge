...
foreach ($file in $htmlFiles) {
    try {
        ...
        if ($content -match "</body>\s*</html>") {
            ...
            Write-Host "  Skipped - unusual file structure" -ForegroundColor Yellow
        }
...
if ($errorCount -eq 0) {
    ...
    Write-Host ""
    Write-Host "Some files had errors. Check the backup folder for originals." -ForegroundColor Yellow
}
