...
| HRM inbox | `/hrm-inbox`: hospital report HL7/FHIR ingest, review queue, **File to chart** |
| Provincial hub settings | `/provincial-hub-settings`: per-org HRM, DHDR, ConnectingOntario, SMART URLs; gateway merge at runtime |
...
| Capital deployment detail | `/capital-deployment-detail` itemized seed outflows by tranche with subtotals (linked from Term Sheet) |
...
| Certification path | `/ontario-readiness#certification-path` forward plan (Phases A–D, timeline, benchmark costs ~$60k–$200k+) |
