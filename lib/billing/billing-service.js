...
class BillingService {
  ...
  async submitClaim(claim, options = {}) {
    ...
    const result = await claims.submitClaimToPortal({
      ...
      portalUrl,
      transport,
      enabled: enabled && transport !== 'manual',
      mcedtConfig: options.mcedtConfig
    });
  ...
  buildMcedtBatch(claimRecords, submitter) {
    return claimsWorkflow.buildBatchFromClaimRecords(claimRecords, submitter);
  }

  validateMcedtBatch(batch) {
    return mcedtFormat.validateClaimBatch(batch);
  }

  serializeMcedtXml(batch) {
    return mcedtFormat.serializeBatchToXml(batch);
  }

  async checkOhipEligibility(patient, options = {}) {
    const client = createMcedtClientFromConfig({
      enabled: options.enabled,
      billing: { claimPortalUrl: options.portalUrl, mcedt: options.mcedtConfig || {} }
    });
