...
const { createMcedtClientFromConfig } = require('./mcedt-client');
 ...
 */
async function submitClaimToPortal({ draft, portalUrl, transport, enabled, mcedtConfig }) {
  if (draft.claimType === 'private' || transport === 'X12_837P') {
  ...
  const batch = mcedtFormat.buildMcedtBatch({
    claims: [mcedtFormat.normalizeClaim(draft)],
    submitter: {
      billingNumber: draft.provider?.billingNumber,
      organizationId: draft.provider?.organizationId
    },
    payerCode: draft.payerCode || 'OHIP',
    province: draft.province || 'ON'
  });

  const client = createMcedtClientFromConfig({
  ...
  if (result.queued) {
    return { draft, batch, submitted: false, queued: true, reason: result.reason || 'portal not configured' };
  }
  if (result.error) {
    return { draft, batch, submitted: false, queued: false, error: result.error, validation: result.validation };
  }
  return {
    draft,
    batch,
    ...
    portalUrl: result.portalUrl || portalUrl,
    externalClaimId: result.externalBatchId || `EXT-${Date.now()}`,
    submittedAt: result.submittedAt || new Date().toISOString()
  };
}

module.exports = {
  buildProvincialClaimDraft,
  buildPrivateClaimDraft,
  submitClaimToPortal
};
