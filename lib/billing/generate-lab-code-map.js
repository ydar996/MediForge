...
function buildProvinceMap({ ref, province, labServices, imagingTests }) {
  ...
  for (const svc of labServices) {
    ...
    if (fee) {
      const storedFee = fee === 'PRIVATE' ? 'PRIVATE' : fee;
      byLabCode[svc.code] = storedFee;
      const normName = normalizeTestName(svc.name);
      if (normName) byTestName[normName] = storedFee;
      if (cpt && !String(cpt).includes('/')) byCpt[String(cpt).split('/')[0]] = storedFee;
      if (cpt && String(cpt).includes('/')) byPanelCpt[normalizePanelCpt(cpt)] = storedFee;
    } else {
