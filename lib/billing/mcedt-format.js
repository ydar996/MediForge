...
/**
 * Structural XML checks for MCEDT batch export (no XSD file; validates required elements).
 */
function validateBatchXmlStructure(xml) {
  const errors = [];
  if (!xml || typeof xml !== 'string') {
    errors.push('XML string is required');
    return { valid: false, errors, claimCount: 0 };
  }
  const trimmed = xml.trim();
  if (!trimmed.startsWith('<?xml')) errors.push('Missing XML declaration');
  if (!/<McedtClaimBatch[\s>]/.test(trimmed)) errors.push('Root element McedtClaimBatch is required');
  if (!/<Submitter>/.test(trimmed)) errors.push('Submitter block is required');
  if (!/<Claims>/.test(trimmed)) errors.push('Claims block is required');
  const claimCount = (trimmed.match(/<Claim[\s>]/g) || []).length;
  if (claimCount < 1) errors.push('At least one Claim element is required');
  if (claimCount > 0) {
    if (!/<PHN>[\s\S]*?<\/PHN>/.test(trimmed)) errors.push('Patient PHN element is required in claims');
    if (!/<ServiceLines>/.test(trimmed)) errors.push('ServiceLines block is required');
    if (!/<FeeCode>/.test(trimmed)) errors.push('FeeCode is required in service lines');
    if (!/<ServiceDate>/.test(trimmed)) errors.push('ServiceDate is required in claims');
  }
  return { valid: errors.length === 0, errors, claimCount };
}
