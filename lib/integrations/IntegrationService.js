...
class IntegrationService {
  ...
  _hrmConsentBlock(hrmConsentGranted) {
    const hrmConsent = require('../interop/hrm-consent');
    const requireConsent = this.config.security?.requireConsent !== false;
    if (!requireConsent || hrmConsentGranted === true) return null;
    const denied = hrmConsent.assertHrmConsent([], { requireConsent: true });
    return denied.blocked ? { queued: true, blocked: true, code: denied.code, message: denied.message } : null;
  }

  _dhdrConsentBlock(dhdrConsentGranted) {
    const dhdrConsent = require('../interop/dhdr-consent');
    const requireConsent = this.config.security?.requireConsent !== false;
    if (!requireConsent || dhdrConsentGranted === true) return null;
    const denied = dhdrConsent.assertDhdrConsent([], { requireConsent: true });
    return denied.blocked ? { queued: true, blocked: true, code: denied.code, message: denied.message } : null;
  }

  async ingestHrmReport({ rawHl7, fhirBundle, organizationId, userId, hrmConsentGranted }) {
    const consentBlock = this._hrmConsentBlock(hrmConsentGranted);
    if (consentBlock) return consentBlock;
    const hrmAdapter = require('../interop/adapters/hrm-adapter');
    let document;
    let standard = 'hl7';
    if (fhirBundle) {
      const parsed = hrmAdapter.fhirDocumentReferenceToChart(fhirBundle);
      if (!parsed.parsed) return parsed;
      document = parsed.document;
      standard = 'fhir';
    } else {
      const payload = hrmAdapter.parseHrmHl7(rawHl7);
      document = hrmAdapter.hrmPayloadToChartDocument(payload);
    }
    await logAudit(this.supabase, {
      organizationId,
      direction: 'inbound',
      standard,
      messageType: 'ingestHrmReport',
      correlationId: document.placerId,
      status: 'received',
  ...
  async fileHrmReportToChart({ reportId, patientId, organizationId, userId }) {
    const hrmChartFile = require('../interop/hrm-chart-file');
    const hrmWorkflow = require('../interop/hrm-workflow');
    if (!this.supabase) throw new Error('Supabase required to file HRM report to chart');
    if (!reportId || !patientId) throw new Error('reportId and patientId are required');
    ...
    const { data: report, error: reportError } = await this.supabase
      ...
      .single();
    if (reportError || !report) throw new Error('HRM report not found');
    ...
    let { data: patient } = await this.supabase
      ...
      .maybeSingle();
    if (!patient) {
      ({ data: patient } = await this.supabase
        .from('patients')
        .select('id, patient_id, unstructured_records')
        .eq('organization_id', organizationId)
        .eq('id', pid)
    ...
    const document = {
      title: report.report_title || 'Hospital Report',
      body: report.report_body || '',
      source: 'hrm',
      placerId: report.placer_id
    };
    const record = hrmChartFile.buildUnstructuredRecordFromReport(report, document);
    const merged = hrmChartFile.mergeUnstructuredRecords(patient.unstructured_records, [record]);

    const { error: patientError } = await this.supabase
      .from('patients')
      .update({
        unstructured_records: merged,
    ...
    const patch = hrmWorkflow.buildFilePatch(document);
    const { error: reportUpdateError } = await this.supabase
      .from('hrm_inbound_reports')
      .update({
        ...patch,
    ...
    await logAudit(this.supabase, {
      ...
      direction: 'inbound',
      standard: 'hrm',
      messageType: 'fileHrmReportToChart',
      correlationId: report.placer_id || reportId,
      status: 'processed',
      payload: { recordId: record.id, patientId: patient.patient_id || patient.id },
      userId,
      province: this.config.province,
      transport: 'chart'
    });
  ...
  queryDhdrMedications({ patient, organizationId, userId, dhdrConsentGranted, fhirBundle }) {
    const consentBlock = this._dhdrConsentBlock(dhdrConsentGranted);
    if (consentBlock) return consentBlock;
    const dhdr = require('../interop/dhdr');
    if (fhirBundle) {
      return { medications: dhdr.parseMedicationStatementBundle(fhirBundle), source: 'bundle' };
    }
    const query = dhdr.buildDhdrQueryUrl({ patient, config: this.config });
    return { ...query, queued: query.queued !== false };
  }

  saveProvincialHubConfig(patch) {
    const cfg = { ...this.config };
    cfg.connectingOntario = { ...(cfg.connectingOntario || {}), ...(patch.connectingOntario || {}) };
    cfg.dhdr = { ...(cfg.dhdr || {}), ...(patch.dhdr || {}) };
    cfg.hrm = { ...(cfg.hrm || {}), ...(patch.hrm || {}) };
    return { saved: true, config: cfg, note: 'Persist org settings via provincial-hub-settings UI / organizations.settings' };
  }
  ...
  exportMcedtXml({ batch }) {
    ...
    const xmlValidation = billing.mcedtFormat.validateBatchXmlStructure(xml);
    return { validation, xml, xmlValidation };
  }

  applyMohRejection({ claim, rejection }) {
    const claimsWorkflow = require('../billing/claims-workflow');
    return claimsWorkflow.applyRejection(claim, rejection);
  }

  validateMcedtBatch({ batch }) {
    const validation = billing.mcedtFormat.validateClaimBatch(batch);
    const xml = billing.mcedtFormat.serializeBatchToXml(batch);
    const xmlValidation = billing.mcedtFormat.validateBatchXmlStructure(xml);
    return { validation, xmlValidation, xmlLength: xml.length };
  }
  ...
  async dicomweb({ operation, params }) {
    ...
    const gatewayUrl = cfg.dicomweb?.dimseGatewayUrl;
    ...
    if (!root || String(root).startsWith('REPLACE_')) {
      throw new Error('dicomweb root not configured');
    }
