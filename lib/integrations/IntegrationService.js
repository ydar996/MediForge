...
class IntegrationService {
   ...
   */
  async sendOrder({ type, patient, order, organizationId, userId }) {
   ...
   */
  async receiveResult({ rawHl7, fhirBundle, organizationId, userId, orderId }) {
   ...
   */
  async sendPrescription({ patient, prescription, organizationId, userId }) {
    if (!patient || !prescription) throw new Error('patient and prescription required');
    const cfg = this.config;
    const payload = interop.adapters.rx.prescriptionToInfowayPayload({ patient, prescription, config: cfg });
    const correlationId = prescription.prescription_number || prescription.id;

    const execute = async () => {
      const rxUrl = cfg.fhir?.rxEndpoint || cfg.rx?.pharmacyEndpoint || cfg.fhir?.baseUrl;
      if (cfg.enabled && rxUrl) {
        ...
        return { payload, result, queued: false };
      }
   ...
   */
  async submitClaim({ patient, provider, invoice, services, payerCode, organizationId, userId }) {
    const cfg = this.config;
    ...
    const execute = async () => {
      if (cfg.enabled && portalUrl && transport !== 'manual') {
        // Production: POST to MCEDT/Teleplan/HLINK web service with clinic certificate
        return {
          draft,
          transport,
          submitted: true,
      ...
      return { draft, transport, submitted: false, queued: true };
    };
    ...
    await logAudit(this.supabase, {
      ...
      payload: { payerCode, transport: result.transport },
      userId,
      province: cfg.province,
      transport
    });
