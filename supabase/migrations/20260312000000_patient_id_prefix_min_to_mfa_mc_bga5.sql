-- Migration: Patient ID prefix MIN→MFA-MCXXXX for organization MIN-2026-BGA5
-- Purpose: Change patient numbering from MINXXXX to MFA-MCXXXX (retroactive + future)
-- NOTE: Org code is MIN-2026-BGA5 (was incorrectly MIN-2026-44JA in prior migration)
-- Runs in single transaction - all or nothing
-- Only updates tables where patient_id is TEXT; skips tables with UUID patient_id
...
BEGIN
  ...
  WHERE org_code = 'MIN-2026-BGA5';

  IF v_org_id IS NULL THEN
    RAISE NOTICE 'Organization with org_code MIN-2026-BGA5 not found. Skipping migration.';
    RETURN;
  END IF;
  ...
  RAISE NOTICE 'Updated org % with patient_id_prefix=MFA-MC, patient_id_previous_prefix=MIN', v_org_id;
END $$;

-- 2. Update patients and cascade to tables with TEXT patient_id only
DO $$
DECLARE
  v_org_id UUID;
  v_old_id TEXT;
...
BEGIN
  ...
  -- Update patients (source of truth): MIN0001 -> MFA-MC0001
  ...
  RAISE NOTICE 'MIN->MFA-MC migration completed for org MIN-2026-BGA5 (id: %)', v_org_id;
END $$;
