-- Race-only demographics: ensure race column, backfill, update intake approval RPC, drop legacy tribe column.
...
-- Backfill race from legacy tribe column (safe if tribe column absent — no rows match)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
...
begin
  ...
  -- Accept race from payload; legacy queued submissions may still use tribe key in JSON only
  v_race := nullif(
    coalesce(
...
ALTER TABLE public.patients DROP COLUMN IF EXISTS tribe;
